...
User Function FGMENU3(lAdmin, cUser)
...
Private c_User		:= IIF(cUser==Nil, Space(15), cUser)
...
//If l_Admin
	MENU oMenu POPUP   2
		...
		MENUITEM "Executar" 	 		ACTION fOptMenu(9)
	ENDMENU
//Endif

MntArray()
//DbTree():New(<nTop>, <nLeft>, <nBottom>, <nRight>, <oWnd>,<{uChange}>, <{uRClick}>, <.lCargo.>, <.lDisable.> )
oTree1 := dbTree():New(25,10,245,300,oDlgTree,,{|o,X,Y| oMenu:Activate(X-210,Y-200,oTree1 )},.T.)
MntTree(.F.)
...
Static Function MntArray()
	...
	While ZZR->(!Eof()) .And. ZZR->(ZZR_FILIAL + ZZR_MODULO) == xFilial("ZZR")+c_Modulo
		...
		aAdd(a_Menu, {ZZR->ZZR_CHAVE, Space(10), ZZR->ZZR_FUNCAO, ZZR->ZZR_TITPOR, ZZR->(Recno())})
		ZZR->(DbSkip())
...
User Function OrdTreeKey(c_Modulo)
	...
	While QRY->(!Eof())
		If c_Pai == Left(QRY->ZZR_CHAVE, 7)
			...
			If AllTrim(QRY->ZZR_CHAVE) <> c_NewOrder
				ZZR->(DbGoTo(QRY->RECZZR))
				ZZR->(RecLock("ZZR", .F.))
					ZZR->ZZR_CHAVE := c_NewOrder
				ZZR->(MsUnLock())
			Endif
			QRY->(DbSkip())
		Else
		...
		If c_Pai == Left(QRY->ZZR_CHAVE, 4)
			...
			If AllTrim(QRY->ZZR_CHAVE) <> c_NewOrder
				If AllTrim(QRY->ZZR_FUNCAO) == ""
					fChaveDePara(QRY->ZZR_CHAVE, c_NewOrder, ZZR->ZZR_FILIAL, ZZR->ZZR_MODULO)
				Else
					ZZR->(DbGoTo(QRY->RECZZR))
					ZZR->(RecLock("ZZR", .F.))
						ZZR->ZZR_CHAVE := c_NewOrder
