...
Private cTipoAux    //incluida a linha [Mauro Nagata, Actual Trend, 28/10/10]
...
cTipoAux := cTipo     //incluida a linha [Mauro Nagata, Actual Trend, 28/10/10]
If cTipo == "1"
	//⁄ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒø
	//≥Posiciona NF                                                            ≥
	//¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ
	...
	If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
		...
		Else
			...
			While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				//condicao constava no fonte anterior a atualizacao da build em 29/10/10 [Mauro Nagata, Actual Trend, 29/10/10]   
				If !AllTrim(SC5->C5_XINFCNF) $ cMensCli
					cMensCli += " " + AllTrim(SC5->C5_XINFCNF)
				EndIf     
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			#IFDEF TOP
				...
				If SF1->(FieldPos("F1_STATUS")) > 0 .And. SD1->(FieldPos("D1_TESACLA"))> 0 .And. SF1->F1_STATUS='C' 
					BeginSql Alias cAliasSD1			
						...
						D1_LOTECTL,D1_NUMLOTE,D1_CUSTO,D1_XDI,D1_XDTDI,D1_XLOCDES,D1_XUFDES
						FROM %Table:SD1% SD1
						WHERE
				...
				Else
					BeginSql Alias cAliasSD1
						...
						D1_SEGURO,D1_DESPESA,D1_CODISS,D1_VALISS,D1_VALIPI,D1_ICMSRET,D1_VUNIT,D1_CLASFIS,
						D1_VALICM,D1_TIPO_NF,D1_PEDIDO,D1_ITEMPC,D1_VALIMP5,D1_VALIMP6,D1_BASEIRR,D1_VALIRR,D1_TESACLA
						D1_TESACLA,D1_LOTECTL,D1_NUMLOTE,D1_CUSTO,D1_XDI,D1_XDTDI,D1_XLOCDES,D1_XUFDES
						FROM %Table:SD1% SD1
						WHERE
						SD1.D1_FILIAL = %xFilial:SD1% AND
						SD1.D1_SERIE = %Exp:SF1->F1_SERIE% AND
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				//≥Adicionado por Jean em 14/09/2010                   ≥
				//¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ

				If !ALLTRIM(SF1->F1_XOBSNF) $ cMensCli
					cMensCli += "  " + ALLTRIM(SF1->F1_XOBSNF)
				EndIf

				// 				Removido para nao tratar os descontos na nota de importacao	         		
				...
				aadd(aProd,	{Len(aProd)+1,;
								...
								SB1->B1_UM,;
								(cAliasSD1)->D1_QUANT,;
								IIF(!(cAliasSD1)->D1_TIPO$"IP".And.((cAliasSD1)->D1_CUSTO <> 0),IIF(!(cAliasSD1)->D1_TIPO$"D",Iif((cAliasSD1)->D1_TIPO$"B",(cAliasSD1)->D1_TOTAL,(cAliasSD1)->D1_TOTAL-(cAliasSD1)->D1_VALDESC),(cAliasSD1)->D1_TOTAL),0),;
								IIF(Empty(SB5->B5_UMDIPI),SB1->B1_UM,SB5->B5_UMDIPI),;
				...
				If SubStr((cAliasSD1)->D1_CF,1,1) == '3'
					...
					DbSelectArea("SWN")
 					DbSetOrder(2) //WN_FILIAL+WN_DOC+WN_SERIE+WN_FORNECE+WN_LOJA
 					If DbSeek(xFilial("SWN")+'000000001'+'UNI'+'000001'+'01')
						...
						If !cTipoNF$"IPC"
							...
							aadd(aDI,(GetNFEIMP(.T.,cDocEnt,cSerEnt,cFornece,cLojaEnt,cTipoNFEnt,cPedido,cItemPC)))
							aDI[Len(aDI)][01][03] := SB1->B1_POSIPI
						Else
							//⁄ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒø
							//≥Tratamento para TAG ImportaÁ„o quando existe a integraÁ„o com a EIC  (Se a nota for complementar)     |
							//¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒŸ
							aadd(aDI,(GetNFEIMP(.T.,cNFOri,cSerOri,cFornece,cLojaEnt,cTipoNFEnt, ,alltrim(Str(VAL(cItemOri))))))
							aDI[Len(aDI)][01][03] := SB1->B1_POSIPI
						EndIf
						aAdi := aDI	 
					EndIf				
...
//[Mauro Nagata, Actual Trend, 28/10/10]
//bloco de instrucao excluido e gerado um novo bloco de instrucao abaixo
//Motivo:Geracao do XML, as vezes a IE do fornecedor nao era incluso no arquivo XML, qdo deveria ser incluso
//Acao:Diferenciar se eh ou nao eh um documento de devolucao ou beneficiamento
// *** If SA1->A1_CONTRIB=="2"
// ***  cString += '<IE></IE>'
// *** Else
// ***   cString += '<IE>'+ConvType(aDest[14])+'</IE>'
// *** EndIf
//[inicio] [Mauro Nagata, Actual Trend, 28/10/10]                                    
If cTipoAux == "1"    //documento de saida
   If SF2->F2_TIPO $ "DB"   //devolucao ou beneficiamento
      SA2->(DbSeek(xFilial("SA2")+SF2->F2_CLIENTE+SF2->F2_LOJA))
      If Empty(SA2->A2_INSCR)    //inscricao estadual nao fornecida
         cString += '<IE></IE>'                             
      Else                                                  
	     cString += '<IE>'+ConvType(aDest[14])+'</IE>'     
      EndIf     
   Else                                                      
      If SA1->A1_CONTRIB=="2"      //contribuinte 1-Sim, 2-Nao
         cString += '<IE></IE>'
      Else
	     cString += '<IE>'+ConvType(aDest[14])+'</IE>'
      EndIf
...
Else  //documento de entrada
   If SF1->F1_TIPO $ "DB"       //devolucao ou beneficiamento
      If SA1->A1_CONTRIB=="2"      //contribuinte 1-Sim, 2-Nao
         cString += '<IE></IE>'
      Else
         cString += '<IE>'+ConvType(aDest[14])+'</IE>'
   ...
   EndIf   
EndIf  
//[fim] [Mauro Nagata, Actual Trend, 28/10/10]   
...
If  !lIssQn    
	...
	Else		
		...
		If Len(aIcms)>0
			...
			cString += '</cpl>'
			cString += '<Tributo>'
			If aICMS[02]=="51"//Enquanto n„o houver adequaÁ„o da estrtura do ICMS51-Diferimento, os casos de diferimento parcial ser„o informados no ICMS90-Outros
				cString += '<CST>90</CST>' 		
			Else
				cString += '<CST>'+ConvType(aICMS[02])+'</CST>'
			Endif
	   		IF(SD2->D2_CLASFIS$ '040,041,050' .And. cVerAmb== "2.00") 
	   			cString += '<vBC>'+'0'+'</vBC>'     
