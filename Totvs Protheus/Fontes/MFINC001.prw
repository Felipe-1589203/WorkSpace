...
//	EndIf
	...
	oProcess := MsNewProcess():New({|lEnd| CriaAcols(oProcess)},"Demonstrativo de Resultados - MUL-T-LOCK","Preparando Roteiro ... ",.F.)
	oProcess:Activate()
...
Static Function CriaAcols(oObj)
	...
	n_r1at++
	oObj:IncRegua1("Carregando a Mov. Bancária .... passo "+Strzero(n_r1at,2)+" de "+StrZero(n_tr1,2))
	oObj:SetRegua2(len(a_tmp))
	For n_y := 2 to len(a_tmp)
	                          
			n_posacol := Ascan(aCols	, {|x| x[len(aHeader)] == a_tmp[n_y,1]})
			n_posahed := Ascan(aHeader	, {|x| AllTrim(x[1]) == DtoC(a_tmp[n_y,2])})
			
			n_sinal := 1
//			IF AllTrim(a_tmp[n_y,4]) $ 'ES'
//				n_sinal := -1
//			EndIf
			...
			aCols[n_posacol, n_posahed]			+= a_tmp[n_y,3] * n_sinal
		   	oObj:IncRegua2("Processando ....")
	Next

	n_r1at++
	oObj:IncRegua1("Selecionando Previsao .... passo "+Strzero(n_r1at,2)+" de "+StrZero(n_tr1,2))

	c_Query := " select     E1_TIPO, E1_NATUREZ, E1_VENCREA as E1_VENCTO, sum(E1_VALOR) as E1_SALDO" + c_EOF
	c_Query += " from       "+RetSqlName("SE1")+" SE1" + c_EOF
	c_Query += " where      E1_VENCREA > '"+DtoS(ddatabase)+"'" + c_EOF
	c_Query += " and        E1_TIPO <> 'RA'" + c_EOF
	c_Query += " and        SE1.D_E_L_E_T_ <> '*'" + c_EOF
	c_Query += " group by   E1_TIPO, E1_NATUREZ, E1_VENCREA " + c_EOF
	
	c_Query += " union all" + c_EOF

	c_Query += " select     E2_TIPO, E2_NATUREZ, E2_VENCREA as E2_VENCTO, sum(E2_VALOR) as E2_SALDO" + c_EOF
	c_Query += " from       "+RetSqlName("SE2")+" SE2" + c_EOF
	c_Query += " where      E2_VENCREA > '"+DtoS(ddatabase)+"'" + c_EOF
	c_Query += " and        SE2.D_E_L_E_T_ <> '*'" + c_EOF
	c_Query += " group by   E2_TIPO, E2_NATUREZ, E2_VENCREA " + c_EOF

	memowrite("CFIN001.sql", c_Query)
	...
	TCSetField("QRY", "E1_SALDO"	, "N", 14, 2)
	...
	QRY->(DbGotop())
	oObj:SetRegua2(n_cont)

	n_r1at++
	oObj:IncRegua1("Processando Previsao .... passo "+Strzero(n_r1at,2)+" de "+StrZero(n_tr1,2))

	While QRY->(!EOF())
	   	oObj:IncRegua2("Processando ....")
		n_posacol := Ascan(aCols	, {|x| x[len(aHeader)] == QRY->E1_NATUREZ})
		n_posahed := Ascan(aHeader	, {|x| AllTrim(x[1]) == DtoC(QRY->E1_VENCTO)})
		
		n_sinal := 1
		
		IF AllTrim(QRY->E1_TIPO) $ 'PA/NDF'
			n_sinal := -1
		EndIf
		...
		aCols[n_posacol, n_posahed]			+= n_sinal * QRY->E1_SALDO
	...
	For n_x := 1 to len(a_dados)	
		If !Empty(SubStr(a_dados[n_x, 1], 1, 1)) .and. SubStr(a_dados[n_x, 1], 1, 1) <> 'S' .and. SubStr(a_dados[n_x, 1], 1, 1) <> 'T'
			a_dados[n_x, 1] := Space(len(rtrim(a_dados[n_x, 1])) - len(alltrim(a_dados[n_x, 1]))) + AllTrim(SubStr(a_dados[n_x, 1], at('-', a_dados[n_x, 1])+1, len(a_dados[n_x, 1])))
		   	faddxml(a_dados[n_x], "s167")
		ElseIf SubStr(a_dados[n_x, 1], 1, 1) == 'T'
			a_dados[n_x, 1] := Space(len(rtrim(a_dados[n_x, 1])) - len(alltrim(a_dados[n_x, 1]))) + AllTrim(SubStr(a_dados[n_x, 1], at('-', a_dados[n_x, 1])+1, len(a_dados[n_x, 1])))
		   	faddxml(a_dados[n_x], "s150")
		Else
			a_dados[n_x, 1] := Space(len(rtrim(a_dados[n_x, 1])) - len(alltrim(a_dados[n_x, 1]))) + AllTrim(SubStr(a_dados[n_x, 1], at('-', a_dados[n_x, 1])+1, len(a_dados[n_x, 1])))
		   	faddxml(a_dados[n_x], "s149")
	   	EndIf
	   	oObj:IncRegua2("Processando ....")
	Next
...
Static Function ExpXML()
	...
	Local cArqSaida := "C:\FLuxo\flcx.xml"
	Private nHdlSaida  := 0
	...
	oExcelApp:WorkBooks:Open( "C:\FLuxo\flcx.xml" ) // Abre uma planilha
...
Static Function fCabecxml()
	...
	Aadd(a_xml, '  <LastAuthor>Actual Trend - www.actualtrend.com.br</LastAuthor>')
	...
	Aadd(a_xml, '  </Style>')
	Aadd(a_xml, '  <Style ss:ID="s147" ss:Parent="s16">')
	...
	Aadd(a_xml, '  </Style>')
	Aadd(a_xml, '  <Style ss:ID="s148" ss:Parent="s16">')
	Aadd(a_xml, '   <Font ss:FontName="Calibri" x:Family="Swiss" ss:Size="11" ss:Color="#000000"/>')
	Aadd(a_xml, '   <NumberFormat ss:Format="_(* #,##0_);_(* \(#,##0\);_(* &quot;-&quot;??_);_(@_)"/>')
	Aadd(a_xml, '  </Style>')
	Aadd(a_xml, '  <Style ss:ID="s149" ss:Parent="s16">')
	Aadd(a_xml, '   <Font ss:FontName="Calibri" x:Family="Swiss" ss:Size="6" ss:Color="#000000"/>')
	Aadd(a_xml, '   <NumberFormat ss:Format="_(* #,##0_);_(* \(#,##0\);_(* &quot;-&quot;??_);_(@_)"/>')
	Aadd(a_xml, '  </Style>')
	Aadd(a_xml, '  <Style ss:ID="s150" ss:Parent="s16">')
	Aadd(a_xml, '   <Font ss:FontName="Calibri" x:Family="Swiss" ss:Size="6" ss:Color="#000000"')
	Aadd(a_xml, '    ss:Bold="1"/>')
	Aadd(a_xml, '   <Interior ss:Color="#CCFFCC" ss:Pattern="Solid"/>')
	Aadd(a_xml, '   <NumberFormat ss:Format="_(* #,##0_);_(* \(#,##0\);_(* &quot;-&quot;??_);_(@_)"/>')
	Aadd(a_xml, '  </Style>')
	Aadd(a_xml, '  <Style ss:ID="s153" ss:Parent="s16">')
	Aadd(a_xml, '   <Font ss:FontName="Calibri" x:Family="Swiss" ss:Size="11" ss:Color="#000000"')
	Aadd(a_xml, '    ss:Bold="1"/>')
	Aadd(a_xml, '   <NumberFormat ss:Format="_(* #,##0_);_(* \(#,##0\);_(* &quot;-&quot;??_);_(@_)"/>')
	Aadd(a_xml, '  </Style>')
	Aadd(a_xml, '  <Style ss:ID="s167" ss:Parent="s16">')
	Aadd(a_xml, '   <Font ss:FontName="Calibri" x:Family="Swiss" ss:Size="6" ss:Color="#000000"')
	Aadd(a_xml, '    ss:Bold="1"/>')
	Aadd(a_xml, '   <Interior ss:Color="#B8CCE4" ss:Pattern="Solid"/>')
	Aadd(a_xml, '   <NumberFormat ss:Format="_(* #,##0_);_(* \(#,##0\);_(* &quot;-&quot;??_);_(@_)"/>')
	Aadd(a_xml, '  </Style>')
	Aadd(a_xml, '  <Style ss:ID="s199" ss:Parent="s16">')
	Aadd(a_xml, '   <Alignment ss:Horizontal="Center" ss:Vertical="Bottom"/>')
	Aadd(a_xml, '   <Font ss:FontName="Calibri" x:Family="Swiss" ss:Size="11" ss:Color="#000000"')
	Aadd(a_xml, '    ss:Bold="1"/>')
	Aadd(a_xml, '   <NumberFormat ss:Format="_(* #,##0_);_(* \(#,##0\);_(* &quot;-&quot;??_);_(@_)"/>')
	Aadd(a_xml, '  </Style>')
	Aadd(a_xml, '  <Style ss:ID="s215" ss:Parent="s16">')
	Aadd(a_xml, '   <Font ss:FontName="Calibri" x:Family="Swiss" ss:Size="6" ss:Color="#000000"')
	Aadd(a_xml, '    ss:Bold="1"/>')
	...
	Aadd(a_xml, ' </Styles>')
	Aadd(a_xml, ' <Names>')
	Aadd(a_xml, '  <NamedRange ss:Name="Database" ss:RefersTo="=sc039990!R2C1:R126C34"/>')
	Aadd(a_xml, ' </Names>')
	Aadd(a_xml, ' <Worksheet ss:Name="sc039990">')
	Aadd(a_xml, '  <Table ss:ExpandedColumnCount="34" ss:ExpandedRowCount="250" x:FullColumns="1"')
	Aadd(a_xml, '   x:FullRows="1" ss:StyleID="s148" ss:DefaultRowHeight="9">')
	Aadd(a_xml, '   <Column ss:StyleID="s148" ss:Width="176.25"/>')
	Aadd(a_xml, '   <Column ss:StyleID="s153" ss:AutoFitWidth="0" ss:Width="45"/>')
	Aadd(a_xml, '   <Column ss:StyleID="s148" ss:AutoFitWidth="0" ss:Width="45" ss:Span="30"/>')
	Aadd(a_xml, '   <Column ss:Index="34" ss:StyleID="s153" ss:AutoFitWidth="0" ss:Width="45"/>')
	Aadd(a_xml, '   <Row ss:AutoFitHeight="0" ss:Height="15">')
	Aadd(a_xml, '    <Cell ss:MergeAcross="12" ss:StyleID="s199"><Data ss:Type="String">Demonstrativo de Resultados - MUL-T-LOCK</Data></Cell>')
	...
	For n_t := 1 to len(a_Header)
		Aadd(a_xml, '    <Cell ss:StyleID="s215"><Data ss:Type="String">'+AllTrim(a_Header[n_t, 1])+'</Data><NamedCell')
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
Static Function faddxml(a_linha, c_style)
	...
	Aadd(a_xml, '    <Cell ss:StyleID="s215">'+c_type+c_val+c_ftype+'<NamedCell')
	Aadd(a_xml, '      ss:Name="Database"/></Cell>')
...
Static Function rdxml()
	...
	Aadd(a_xml, '     <ActiveRow>24</ActiveRow>')
	Aadd(a_xml, '    </Pane>')
	Aadd(a_xml, '   </Panes>')
