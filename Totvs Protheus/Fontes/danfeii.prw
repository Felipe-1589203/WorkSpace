...
Private nConsTex 	:= 0.1 // Constante para concertar o cálculo retornado pelo GetTextWidth.
...
//-----------------------------------------------------------------------------------------------------------------------------------------                                                                                                                                                            
//Substituido bloco acima pelo bloco abaixo [Bruno Parreira, Actual Trend, 13/06/2012]
For n_x := 1 to Len(oDet)
	_xcfop := oDet[n_x]:_Prod:_CFOP:TEXT
Next

If SF2->F2_TIPO $ "I" .And. alltrim(_xcfop) $ "5401/6401"
   aTotais[01] := ""
   aTotais[02] := ""                                                                   
   aTotais[03] := ""                                                                   
   aTotais[04] := Transform(Val(oTotal:_ICMSTOT:_vICMS:TEXT),"@ze 9,999,999,999,999.99")
   aTotais[05] := ""
Else
...
Endif
//Fim Bloco [Bruno Parreira, Actual Trend, 13/06/2012]
//-----------------------------------------------------------------------------------------------------------------------------------------
...
//Substituida linha acima pelo bloco abaixo [Bruno Parreira, Actual Trend, 13/06/2012]
If SF2->F2_TIPO $ "I" .And. alltrim(_xcfop) $ "5401/6401"
	aTotais[11] := Transform(Val(oTotal:_ICMSTOT:_vICMS:TEXT),"@ze 9,999,999,999,999.99")                
else
	aTotais[11] := 	Transform(Val(oTotal:_ICMSTOT:_vNF:TEXT),"@ze 9,999,999,999,999.99")
EndIf	
//Fim Bloco [Bruno Parreira, Actual Trend, 13/06/2012]
...
// Linha acima substituida pela linha abaixo. Removido a pedido do Fiscal 07/12/2012 
// Copiado na compatibilizacao de Danfe [Bruno Parreira, Actual Trend, 31/01/2013]
...
Static Function RetTamCol(aCabec, aValores, oPrinter, oFontCabec, oFont)
	...
	For nX := 1 To Len(aCabec)
		...
		AADD(aTamCol, {})
		aTamCol[nX] := CalcFieldSize("C",Len(alltrim(aCabec[nX])),0, ,alltrim(aCabec[nX]),oFontCabec)
	...
	For nX := 1 To Len(aValores[1])
		...
		For nY := 1 To Len(aValores[1][nX])
			...
			If (CalcFieldSize("C",Len(Alltrim(aValores[1][nX][nY])),0, ,Alltrim(aValores[1][nX][nY]),oFontCabec) * nConsTex) > nAux
				nAux := (CalcFieldSize("C",Len(Alltrim(aValores[1][nX][nY])),0, ,Alltrim(aValores[1][nX][nY]),oFontCabec) * nConsTex) + 30
			EndIf
...
Static Function RetTamTex(cTexto, oFont, oPrinter)
	...
	nTamanho := CalcFieldSize("C",Len(cTexto),0, ,cTexto,oFont) + 10
