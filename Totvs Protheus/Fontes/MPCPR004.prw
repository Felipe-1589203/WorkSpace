...
//³                                                                        ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oReport:= TReport():New("MPCPR004",cTitle,"MTR820", {|oReport| ReportPrint(oReport, cAliasTop)},OemToAnsi(STR0001)) //"Este programa ira imprimir a Rela‡„o das Ordens de Produ‡„o"
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
*/
User Function R002IMR4()

Local titulo  := STR0039 //"Ordens de Producao"
Local cString := "SC2"
Local wnrel   := "MPCPR004"
...
If !IsLockSH8()
	...
	RptStatus({|lEnd| U_005IMPRI(@lEnd,wnRel,titulo,if(mv_par12 == 2,"P","P")) },titulo)
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
User Function 005IMPRI(lEnd,wnRel,titulo,tamanho)
...
Local limite     := 80
Local nQuant     := 1
Local nomeprog   := "MPCPR004"
Local nTipo      := 18
Local cProduto   := SPACE(LEN(SC2->C2_PRODUTO))
Local cQtd ,cQtd2,cqtd3:=0,cqtdTotal:=0,i,nBegin // Actual LH 28/11/2016  criado variaveis
...
Private aArray   := {}
Private li       := 80
Public numcabec // actual lh 28/11/2016 criado variavel para tratar impressao do cabeçalho
cAliasTop  := "SC2"
...
While !Eof()
	...
	For I := 2 TO Len(aArray)
	  ...
	  //	For nBegin := 1 To Len(Alltrim(aArray[I][2])) Step 31
			@li,016 PSay Substr(aArray[I][2],1,31)
	  //		li++
	 //	Next nBegin
	 //	Li-- 
		cQtd := Alltrim(Transform(aArray[I][5],PesqPictQt("D4_QUANT",TamSX3("D4_QUANT")[1])))
		@Li , (46+11-Len(cQtd)) PSay cQtd					// QUANTIDADE               
		// criado expressao para tratar o total das quantidades que estao em ordem de producao actual LH 28/11/2016
		cQtd2 :=val(cQtd)
		cqtd3+=cqtd2        
		//final da expressao quantidade total é armazenada na variavel cqtd3 Actual LH 28/11/2016
		If mv_par12 == 2
		...
		Else
		 ...
		 //	@li , 102 PSay "| "+aArray[I][11]                  	// SUB-LOTE       
		 	@li , 68 PSay  ((cAliasTop)->(C2_NUM+C2_ITEM+C2_SEQUEN+C2_ITEMGRD))    // Actual LH 28/11/2016 ORDEM PRODUCAO
		EndIf
		Li++
		...
		IF li > 60
			Li := 0  
			//criado expressao IF para tratar a impressao do cabeçalho ... imprimir apenas 1 vez  Actual LH 28/11/2016
		    if empty(numcabec) 
				CabecOp(Tamanho)		// imprime cabecalho da OP
	       endif     
	       //fim da expressao tratando a impressao do cabeçalho Actual LH 28/11/2016
	   EndIF
	...
	*	m_pag++
 //	Li := 0					// linha inicial - ejeta automatico         comentado para nao pular pagina e imprimir na proxima folha  Actual 28/11/2016
	aArray:={}
...
EndDO               
// criada expressao para tratar o total das quantidades no final do relatorio   ACTUAL _ LH 28/11/2016
    cqtdTotal :=cqtd3  
     LI++
	@Li,16 PSay "Quantidade Total de Produto em O.P: " + cvaltochar(cqtdTotal)                 //"Qtde Total ordem de producao.:"
	LI++
// fim da expressao  ACTUAL - LH 28/11/2016
dbSelectArea("SH8")
dbCloseArea()
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
If (cAliasTop)->C2_QUJE + (cAliasTop)->C2_PERDA > 0       
//criado expressao para tratar o cabeçalho com o tipo de segmento e dados do cliente - ACTUAL LH -28/11/2016
	dbSelectArea("SC5")
	dbSetOrder(1)
...
//@Li,26 PSay "Cliente : " +SC5->C5_CLIENTE + " Loja: " +Posicione("SA1",1,xfilial("SA1")+SC5->C5_CLIENTE,"A1_LOJA")+ " Nome : " +Posicione("SA1",1,xfilial("SA1")+SC5->C5_CLIENTE,"A1_NOME")
// fim da tratativa ACTUAL LH 28/11/2016
Else
...
Li++
//@Li,00 PSay cCabec2  // comentado para nao imprimir cabeçalho COMPONENTE Actual  LH 28/11/2016
//Li++
@Li,00 PSay cCabec3
Li++
@Li,00 PSay __PrtFatLine()
