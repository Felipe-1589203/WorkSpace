...
Local cThema        := IIf( (Alltrim(GetTheme()) == "TEMAP10") .Or. SetMdiChild(), "TEMAP10", "")
...
Private cThema      := IIf( (Alltrim(GetTheme()) == "TEMAP10") .Or. SetMdiChild(), "TEMAP10", "")
...
If SX5->(DbSeek(xFilial("SX5")+"24"))
	While SX5->(!Eof()) .And. SX5->X5_TABELA == "24"
		cSitCart += " (CASE WHEN SE1.E1_XTPPAG = '"+ AllTrim(SX5->X5_CHAVE)+"' THEN '"+ AllTrim(SX5->X5_DESCRI)+"' ELSE "
		nSitCont++
		SX5->(DbSkip())
	End-While
...
Else
	cQuery += "  AND SE1.E1_NUMBOR  = '"+ CriaVar("E1_NUMBOR",.F.) +"'"
	cQuery += "  AND ( SE1.E1_SALDO > 0  OR SE1.E1_OCORREN = '02' ) "
	cQuery += "  AND SE1.E1_SITUACA IN ('0','F','G') "
	...
	While ( cAliasA )->( !Eof() )
		...
		For nInd := 1 To ( cAliasA )->( FCount() )
			...
			ElseIf ( cAliasA )->( FieldName( nInd ) ) == 'DIAS'
				...
				Aadd( aAux, DataValida( DataValida(dDataVenct)+nFloatDias ) - DataValida(dDataBorde) )
			Else
...
For _n2 := 1 to Len(oSE1Get:aCols)
	//If lFloat == .T.
	nFloatDias := Val(nFloat)
	dDataVenct := DataValida( oSE1Get:aCols[_n2, GdFieldPos("E1_VENCREA",oSE1Get:aHeader)]  )
	dDataBorde := IIf (Empty(DataValida(oSE1Get:aCols[_n2, GdFieldPos("E1_DATABOR",oSE1Get:aHeader)] )), DataValida(Date()), DataValida( oSE1Get:aCols[_n2, GdFieldPos("E1_DATABOR",oSE1Get:aHeader)]))
	dFeriaVenc := Ctod("")
	dFeriaBord := Ctod("")
	...
	oSE1Get:aCols[_n2, GdFieldPos("DIAS",oSE1Get:aHeader)] :=  DataValida( DataValida(dDataVenct)+nFloatDias ) - DataValida(dDataBorde)
	//EndIf
