...
#DEFINE MAXITEMC  023                                                // Máxima de caracteres por linha de produtos/serviços
...
If ( !oDanfe:lInJob )
	...
	// ----------------------------------------------
	If oSetup:GetProperty(PD_PRINTTYPE) == 1
	...
	ElseIf oSetup:GetProperty(PD_PRINTTYPE) == 2
...
If ( oDanfe:lInJob )
	...
	DanfeProc(@oDanfe,.F.,cIdEnt,cVal1,cVal2)
Else
	// ------------------------------------------------------------------------------------------
...
Static Function DanfeProc(oDanfe,lEnd,cIdEnt,cVal1,cVal2)
...
If SF2->(FieldPos("F2_ESPECI1")) <>0 .Or. SF2->(FieldPos("F2_PLIQUI")) .Or. SF2->(FieldPos("F2_PBRUTO"))
	aadd(aEspVol,{SF2->F2_ESPECI1,Iif(SF2->F2_PLIQUI>0,str(SF2->F2_PLIQUI),""),Iif(SF2->F2_PBRUTO>0, str(SF2->F2_PBRUTO),"")})
Endif
...
For nX := 1 To nLenDet
	...
	While !Empty(cAux) .Or. !Empty(cAuxItem)
		aadd(aItens,{SubStr(cAuxItem,1,12),;
...
If Type("oImposto:_Imposto")<>"U"
	If Type("oImposto:_Imposto:_ICMS")<>"U"
		If Type("oImposto:_IMPOSTO:_ICMS:_ICMS60")<>"U" 
			If oNF:_INFNFE:_VERSAO:TEXT >= "2.00"
	   			cAux := 	"Conforme Art 273 e Art.313 do RICMS o valor de ICMS ST é de R$ "+ oImposto:_IMPOSTO:_ICMS:_ICMS60:_VICMSSTRET:TEXT+ " e o valor da Base de ST é R$" + oImposto:_IMPOSTO:_ICMS:_ICMS60:_VBCSTRET:TEXT 
			Else
	   			cAux := 	"Conforme Art 273 e Art.313 do RICMS o valor de ICMS ST é de R$ "+ oImposto:_IMPOSTO:_ICMS:_ICMS60:_VICMSST:TEXT+ " e o valor da Base de ST é R$" + oImposto:_IMPOSTO:_ICMS:_ICMS60:_VBCST:TEXT
			EndIf
			While !Empty(cAux)
				aadd(aMensagem,SubStr(cAux,1,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN) - 1, MAXMENLIN)))
				cAux := SubStr(cAux,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN), MAXMENLIN) + 1)
...
While nLenItens>0
	nFolhas	++   		
	If nFolhas%2==0 .And. nLenItens>MAXITEMP3 .And. MV_PAR06==1
    	nLenItens	-=	MAXITEMP3
 	ElseIf (nFolhas%2<>0 .Or. MV_PAR06==2) .And. nLenItens>MAXITEMP2
		nLenItens	-=	MAXITEMP2
	Else
		Exit
  	EndIf
...
oDanfe:Say(006, 002, "RECEBEMOS DE "+oEmitente:_xNome:Text+" OS PRODUTOS CONSTANTES DA NOTA FISCAL INDICADA AO LADO", oFont07:oFont)
...
nLinCalc	:=	065
cStrAux		:=	AllTrim(oEmitente:_xNome:Text)
...
cStrAux		:=	AllTrim(oEmitente:_EnderEmit:_xLgr:Text)+", "+AllTrim(oEmitente:_EnderEmit:_Nro:Text)
...
If Type("oEmitente:_EnderEmit:_xCpl") <> "U"
	cStrAux		:=	"Complemento: "+AllTrim(oEmitente:_EnderEmit:_xCpl:TEXT)
	nForTo		:=	Len(cStrAux)/32
	nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
	For nX := 1 To nForTo
		oDanfe:Say(nLinCalc,098,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*32)+1),32),oFont08N:oFont)
...
oDanfe:Say(215,002,oDestino:_XNome:TEXT,oFont08:oFont)
...
oDanfe:Say(382,319,aTransp[02],oFont08:oFont)
...
For nY := 1 To nLenItens
	...
	If lflag
		If nL > MAXITEM
			...
			oDanfe:Say(171,054,"DESCRIÇÃO DO PROD./SER.",oFont10N:oFont)
