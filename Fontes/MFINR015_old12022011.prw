...
/* Aqui
			If TRE->VALOR > 0	           
				...
				aadd(a_TmpRec, {TRE->VALOR, TRE->NATUREZ})
...
*/
	   ...
	   Enddo
		...
		a_totrec := {}
		For n_y := 1 to len(a_TmpRec)
			DbSelectArea('SED')
			DbSetOrder(1)
			If DbSeek(xFilial('SED')+a_TmpRec[n_y, 2])
			 	n_pos := ascan(a_totrec , {|x| x[1] == SED->ED_XNATSIN})
 		 	  	If n_pos <= 0
  		 			aadd(a_totrec, {SED->ED_XNATSIN, a_TmpRec[n_y, 1]})
		 	  	Else
	  				a_totrec[n_pos, 2] += a_TmpRec[n_y, 1]
			 	EndIf
				
  		 	EndIf
		Next
		
		Asort(a_totrec	,,,{ | x,y | x[2] < y[2] } )                       

		For n_x := 1 to len(a_totrec)

		   @ Li, aCol[01] PSAY  GetAdvFval("SED","ED_DESCRIC",Xfilial("SED")+alltrim(a_totrec[n_x,1]),1,"NAO ACHOU") 
		   @ Li, aCol[02] PSAY  Transform(a_totrec[n_x, 2], "@E 99,999,999.99")


		   nTotRec+=a_totrec[n_x, 2]
		   Li++
		   If Li > 55
		      Cabec(Titulo,Cabec1,Cabec2,NomeProg,Tamanho,nTipo)
		   Endif
	   ...
	   Li++
		...
		While SE5->(!EOF()) .and. SE5->E5_DATA >= MV_PAR01 .and. SE5->E5_DATA <= MV_PAR02
			If SE5->E5_RECPAG == 'P' .and. Alltrim(SE5->E5_MOEDA) == 'M1' .and. SE5->E5_SITUACA <> 'C' .and. !empty(SE5->E5_RECONC) 
				aadd(a_tmptst, {SE5->E5_VALOR, SE5->E5_NATUREZ})
			EndIf
...
cQuery3:= "SELECT SE5.E5_FILIAL, SE5.E5_DATA, SE5.E5_TIPO,SE5.E5_MOEDA, " 
cQuery3+= "SE5.E5_VALOR, SED.ED_XNATSIN,SE5.E5_NATUREZ, SE5.E5_BANCO, SE5.E5_AGENCIA, SE5.E5_CONTA, SE5.E5_DOCUMEN, SE5.E5_RECPAG, SE5.E5_TIPODOC " 
cQuery3+= "FROM SE5010 as SE5 "
cQuery3+= "INNER JOIN SED010 as SED "
cQuery3+= "ON SE5.E5_NATUREZ = SED.ED_XNATSIN "
cQuery3+= "WHERE SE5.E5_DATA >= '"+DTOS(mv_par01)+"' " 
cQuery3+= "AND SE5.E5_DATA <= '"+DTOS(mv_par02)+"' "
cQuery3+= "AND SE5.E5_TIPODOC IN ('BD','CB','  ','RA') "
cQuery3+= "AND SE5.E5_SITUACA <> 'C' "   
...
cQuery3+= "AND SE5.E5_TIPO  IN ('RA',' ') " 
cQuery3+= "AND SE5.E5_RECONC  <> ' ' " 
cQuery3+= "AND SE5.E5_NATUREZ IN ('1006010','1006022','1004003','1004005','1005000','1005010','1005011','1005012','1006000','1006011','1006020','1006021','1006028') "  
cQuery3+= "AND SE5.D_E_L_E_T_ <> '*' "
cQuery3+= "ORDER BY SE5.E5_DOCUMEN " 
...
//+------------------------+
	...
	aAdd(_aStru,{"FILIAL"	,"C",02,0}) //GRUPO DO PRODUTO
	aAdd(_aStru,{"DATAE5"	,"D",08,0}) //CLIENTE
	aAdd(_aStru,{"TIPO"		,"C",03,0}) //LOJA
	aAdd(_aStru,{"MOEDA"	,"C",02,0}) //RAZAO SOCIAL
	aAdd(_aStru,{"VALOR"	,"N",17,2}) //PRODUTO
	aAdd(_aStru,{"NATUREZ"	,"C",10,0}) //DESCRICAO DO PRODUTO
	aAdd(_aStru,{"XNATSIN"	,"C",10,0}) //DESCRICAO DO PRODUTO
	aAdd(_aStru,{"BANCO"	,"C",03,0}) //QUANTIDADE REF. MES JANEIRO
	aAdd(_aStru,{"AGENCIA"	,"C",05,0}) //QUANTIDADE REF. MES FEVEREIRO
	aAdd(_aStru,{"CONTA"	,"C",10,0}) //QUANTIDADE REF. MES MARCO
	aAdd(_aStru,{"DOCUMEN"	,"C",50,0}) //QUANTIDADE REF. MES ABRIL
	aAdd(_aStru,{"RECPAG"	,"C",01,0}) //QUANTIDADE REF. MES MAIO
	aAdd(_aStru,{"TIPODOC"	,"C",02,0}) //QUANTIDADE REF. MES JUNHO
