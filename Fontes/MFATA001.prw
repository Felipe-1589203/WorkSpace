...
User Function MFATA001()
	...
	//ณVerifica os empenhos no B2 e BF                    ณ
	//ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤู
	c_Query := "select R_E_C_N_O_ as REC from "+RetSqlName('SB2')+" where B2_LOCAL = '77' and B2_RESERVA <> 0 "
	If Select("QRY") > 0
		DbSelectArea("QRY")
		DbCloseArea()
	EndIf
	...
	EndDo
/*	DbSelectArea('SB2')
	DbSetOrder(2) //B2_FILIAL, B2_LOCAL, B2_COD, R_E_C_N_O_, D_E_L_E_T_
	DbSeek(xFilial('SB2')+'77')
	While SB2->(!EOF()) .and. SB2->B2_LOCAL = '77'
		If SB2->B2_RESERVA <> 0
			RecLock('SB2', .F.)
			SB2->B2_RESERVA := 0
			MsUnLock()
		...
		SB2->(DbSkip())
	EndDo*/
	c_Query := "select R_E_C_N_O_ as REC from "+RetSqlName('SBF')+" where BF_LOCAL = '77' and BF_EMPENHO <> 0 "
	If Select("QRY") > 0
		DbSelectArea("QRY")
		DbCloseArea()
	EndIf
...
/*	DbSelectArea('SBF')
	...
	DbSeek(xFilial('SBF')+'77')
	While SBF->(!EOF()) .and. SBF->BF_LOCAL = '77'
		If SBF->BF_EMPENHO <> 0
			RecLock('SBF', .F.)
			SBF->BF_EMPENHO := 0
			MsUnLock()
