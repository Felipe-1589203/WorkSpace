...
If cTipo == "1"
	...
	//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
	            ...
	            //Bloco abaixo comentado conforme estava comentado na ultima compatibilizacao de nfesefaz. [Bruno Parreira, Actual Trend, 31/01/2013]
	            /*
		    		Else 
		         ...
		         EndIf
			   	...
			   	//⁄ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒø
					...
					//Bloco abaixo comentado pois na Multlock nao se usa o codigo e descricao das amarracoes. [Bruno Parreira, Actual Trend, 11/06/2012]	
					/* 	 
		         ...
		         */
					...
					//-----------------------------------------------------------------------------------------
					If (SF4->(FieldPos("F4_CONSUMO")) > 0 .And. SF4->F4_CONSUMO == "N") .And. (cOrigem $"1-2-3-4-5-6" .And. cCSTrib $ "00-10-20-70-90")
					...
					While CD2->(!Eof()) .And. xFilial("CD2") == CD2->CD2_FILIAL .And.;
						...
						Do Case
							...
							Case AllTrim(CD2->CD2_IMP) == "ISS" 
								...
								//Substituida linha acima pela linha abaixo [Jean Cavalcante, Actual Trend, 02/02/2012]
								aISS[01] += (cAliasSD2)->D2_TOTAL								
								aISS[02] += CD2->CD2_BC
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				//Condicao copiada na ˙ltima atualizacao de build. [Bruno Parreira, Actual Trend, 11/06/2012]
...
If !Empty(aNota)
	...
	For nX := 1 To Len(aProd)
		If nLenaIpi > 0
			...
			EndIf
		EndIf
		cString += NfeItem(aProd[nX],aICMS[nX],aICMSST[nX],aIPI[nX],aPIS[nX],aPISST[nX],aCOFINS[nX],aCOFINSST[nX],aISSQN[nX],aCST[nX],aMed[nX],aArma[nX],aveicProd[nX],aDI[nX],aAdi[nX],aExp[nX],aPisAlqZ[nX],aCofAlqZ[nX],aAnfI[nX],cTipo,cVerAmb, aComb[Nx],@cMensFis,aCsosn[Nx],aPedCom[nX],aNota[5],aICMSZFM[nX],aDest,cIpiCst,cTipo,@nTotTrib,cTpCliente)
	Next nX
	...
	cString += NfeInfAd(cMensCli,cMensFis,aPedido,aExp,cAnfavea,aMotivoCont,aNota,aNfVinc,aProd,aDI,aNfVincRur,aRetido,cNfRefcup,cSerRefcupc,cTipo,nTotTrib,cPercTrib)
	cString += "</infNFe>"
EndIf
...
Static Function NfeItem(aProd,aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,aMed,aArma,aveicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ,aAnfI,cTipo,cVerAmb,aComb,cMensFis,cCsosn,aPedCom,cF2Tipo,aICMSZFM,aDest,cIpiCst,cTipo,nTotTrib,cTpCliente)
...
Static Function NfeInfAd(cMsgCli,cMsgFis,aPedido,aExp,cAnfavea,aMotivoCont,aNfSa,aNfVinc,aProd,aDI,aNfVincRur,aRet,cNfRefcup,cSerRefcup,cTipo,nTotTrib,cPercTrib)
...
If Len(cMsgCli)>0
	...
	IF (SM0->M0_ESTENT) $ "SP" .AND. cTipo=='0'
		SFT->(dbSetOrder(6))
		// Alterado por Ricardo Batagella - 14/03/2013
		If ! Empty(Alltrim(cNfRefcup+cSerRefcup)) // este if foi incluido
