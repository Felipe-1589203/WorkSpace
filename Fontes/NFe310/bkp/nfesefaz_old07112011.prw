...
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥ 24/05/11 ≥ Paulo Oliveira≥Ajustado a rotina de impress„o do cabeÁalho ≥±±
±±≥ na funÁ„o: Static Function NfeDest(aDest,cVerAmb,aTransp,aCST)         ±±
...
Private wPesqSA1 := space(20)
Private wPesqSA2 :=space(20)
If FunName() == "SPEDNFSE"
...
If cTipo == "1"
	...
	If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
		...
		ElseIf FunName() == "SPEDNFSE"
			...
			wPesqSA1 := SF2->F2_CLIENTE+SF2->F2_LOJA
...
//				If !Upper(SA1->A1_EST) == "EX"
			...
			Else
			    dbSelectArea("SA2")   // entrou aqui
				dbSetOrder(1)
				MsSeek(xFilial("SA2")+SF2->F2_CLIENTE+SF2->F2_LOJA)	
				wPesqSA2 :=SF2->F2_CLIENTE+SF2->F2_LOJA
				...
				aadd(aDest,"TEL "+SA2->A2_DDD+SA2->A2_TEL)     
...
If cVerAmb== "2.00"
	cString += '<hSaiEnt>'+ConvType(time())+'</hSaiEnt>
Endif
...
cString += '<IE>'+ConvType(SM0->M0_INSC)+'</IE>'
...
If Len(aDI)>0 .And. ConvType(aDI[04][1]) == "I19"
	...
	If Len(aAdi)>0          //TIREI O COMENTARIO
		cString += '<adicao>'
...
Elseif Len(aDI)>0
	...
	If Len(aAdi)>0     //TIREI O 2 COMENTARIO
		cString += '<adicao>'
