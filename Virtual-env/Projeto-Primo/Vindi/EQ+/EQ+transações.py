...
def fetch_and_process_data(page, starting_after):
    ...
    if response.status_code == 200:
        ...
        columns_to_keep = ["id", "transaction_type", "status", "amount", "created_at", "updated_at", "charge.id","payment_method.name","customer.id","customer.name", "customer.email"]
             ...
             "transaction_type":"tipo-transação",
              "status": "status-transação",
               "amount": "valor-transação",
                ...
                #   "payment_method.public_name": "forma-pagamento",
                     "payment_method.name": "conta",
...
def mapear_payment_method(payment_method):
    if "Conta Primo" in payment_method:
        return "Conta Primo"
    elif "Conta Top Invest" in payment_method:
        return "Conta Top Invest"
    elif "conta Top Invest" in payment_method:
        return "Conta Top Invest"    
    else:
...
final_df['conta'] = final_df['conta'].apply(mapear_payment_method)
...
with ThreadPoolExecutor() as executor:
    while True:
        ...
        if existing_data is not None:
            df = df[df['id-transação'] > ultimo_id_coletado]
