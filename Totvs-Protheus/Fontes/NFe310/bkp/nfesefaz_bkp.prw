...
If cTipo == "1"
	...
	If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
		...
		ElseIf FunName() == "SPEDNFSE"
			...
			aadd(aDest,ConvType(MyGetEnd(SA1->A1_END,"SA1")[3] ))//ConvType(IIF(FisGetEnd(SA1->A1_END)[2]<>0,FisGetEnd(SA1->A1_END)[2],"SN")))
		...
		Else
			...
			If !SF2->F2_TIPO $ "DB" 
				...
				aadd(aDest,ConvType(MyGetEnd(SA1->A1_END,"SA1")[3] ))//ConvType(IIF(MyGetEnd(SA1->A1_END,"SA1")[2]<>0,MyGetEnd(SA1->A1_END,"SA1")[2],"SN")))
				...
				If SF2->(FieldPos("F2_CLIENT"))<>0 .And. !Empty(SF2->F2_CLIENT+SF2->F2_LOJENT) .And. SF2->F2_CLIENT+SF2->F2_LOJENT<>SF2->F2_CLIENTE+SF2->F2_LOJA
					...
					aadd(aEntrega,ConvType(MyGetEnd(SA1->A1_END,"SA1")[3] )) //ConvType(IIF(MyGetEnd(SA1->A1_END,"SA1")[2]<>0,MyGetEnd(SA1->A1_END,"SA1")[2],"SN")))
			...
			Else
				...
				aadd(aDest,ConvType(MyGetEnd(SA2->A2_END,"SA2")[3] ))// ConvType(IIF(MyGetEnd(SA2->A2_END,"SA2")[2]<>0,MyGetEnd(SA2->A2_END,"SA2")[2],"SN")))
			...
			While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				If !AllTrim(SC5->C5_MENNOTA) $ cMensCli
					cMensCli += "N.Pedido: " + SC5->C5_NUM
				EndIf
				If !AllTrim(SC5->C5_MENNOTA) $ cMensFis
					cMensFis += "            " + AllTrim(SC5->C5_MENNOTA)
				EndIf
				If !AllTrim(SC5->C5_XINFCNF) $ cMensFis
					cMensFis += "            " + AllTrim(SC5->C5_XINFCNF)
				EndIf
				...
				If !AllTrim(FORMULA(SC5->C5_MENPAD)) $ cMensCli
				...
				dbSetOrder(1)
				MsSeek(xFilial("SF4")+(cAliasSD2)->D2_TES)				
				If !AllTrim(FORMULA(SF4->F4_FORMULA)) $ cMensCli
			...
			If Len(aIPIDev)>0
				...
				For nX := 1 To Len(aIPIAux)
					...
					cMensCli += "(Valor do IPI: R$ "+cValIPI+" - "+"ClassificaÁ„o fiscal: "+aIPIAux[nX][2]+")"+CRLF
					cValIPI  := ""
					cNCMAux  := ""
				Next nX
			EndIf
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			Else
				...
				aadd(aDest,SA2->A2_NOME)
				aadd(aDest,MyGetEnd(SA2->A2_END,"SA2")[1])
				aadd(aDest,ConvType(MyGetEnd(SA1->A1_END,"SA1")[3] ))//ConvType(IIF(MyGetEnd(SA2->A2_END,"SA2")[2]<>0,MyGetEnd(SA2->A2_END,"SA2")[2],"SN")))
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				//≥Adicionado por alexandre em 06/08/2010              ≥
				...
				aadd(aProd,	{Len(aProd)+1,;
								...
								SB1->B1_UM,;
								(cAliasSD1)->D1_QUANT,;
								IIF(!(cAliasSD1)->D1_TIPO$"IP",Iif(!(cAliasSD1)->D1_TIPO$"D",(cAliasSD1)->D1_TOTAL,(cAliasSD1)->D1_TOTAL),0),;
								IIF(Empty(SB5->B5_UMDIPI),SB1->B1_UM,SB5->B5_UMDIPI),;
					...
					Do Case
						Case AllTrim(CD2->CD2_IMP) == "ICM"
							aTail(aICMS) := {CD2->CD2_ORIGEM,;
											  CD2->CD2_CST,;
											  CD2->CD2_MODBC,; 
							                  IiF(CD2->CD2_PREDBC>0,IiF(CD2->CD2_PREDBC > 100,0,100-CD2->CD2_PREDBC),;// Tratamento para obter o percentual da reduÁ„o de base do icms nota interna e importacao(integracao com EIC)
...
Else
	aRet[1] := cEndereco
	If cAlias = 'SA1'
		If !Empty(&(cAlias+'->A1_XNUM'))                
			aRet[1] := cEndereco
			aRet[3] := &(cAlias+'->A1_XNUM')                                                           
		Else
			aRet := FisGetEnd(cEndereco)
		EndIf
	ElseIf cAlias = 'SA2'                  
		If !Empty(&(cAlias+'->A2_XNUM'))
			aRet[1] := cEndereco
			aRet[3] := &(cAlias+'->A2_XNUM')
		Else
			aRet := FisGetEnd(cEndereco)
		EndIf
	Else
