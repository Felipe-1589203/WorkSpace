    ...
    While !Eof() .And. !lEnd  
	   ...
	   Li++
		...
		While SE5->(!EOF()) .and. SE5->E5_DATA >= MV_PAR01 .and. SE5->E5_DATA <= MV_PAR02
			If SE5->E5_RECPAG == 'P' .and. Alltrim(SE5->E5_MOEDA) == 'M1' .and. SE5->E5_SITUACA <> 'C' .and. !empty(SE5->E5_RECONC) //.and. Alltrim(SE5->E5_NATUREZ) <> '9999999' //.and. SE5->E5_TIPO <> 'RA'
		...
		For n_y := 1 to len(a_tmptst)
			...
			If DbSeek(xFilial('SED')+a_tmptst[n_y, 2])
		 	  ...
		 	  //		aadd(a_totnat, {SED->ED_CODIGO, a_tmptst[n_y, 1]}) //Analitico   Alterar para voltar aglutinar....
...
//	MsProcTxt("Selecionando dados... Por favor aguarde!")
