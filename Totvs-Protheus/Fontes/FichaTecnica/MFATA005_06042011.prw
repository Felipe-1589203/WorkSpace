...
If !aCols[n,Len(aHeader)+1].And.!Empty(aCols[n,n_posprod])
	If !Empty(c_prod) .and. substr(c_prod,1,2) = 'PO'         
		...
		Do While !Eof() .And. (X3_ARQUIVO == "SZ0")
			If !(M->C5_XORIGEM != "C".And.AllTrim(SX3->X3_CAMPO) $ "Z0_BATENTE#Z0_FECHELE#Z0_TESBAT")       //se segmento do cliente for diferente de Corporativo
			   aAdd(aCampoSN8,X3_CAMPO)                   //Campos a serem alterados, exceto os campos chave.
			EndIf   
			If !(M->C5_XORIGEM != "C".And.AllTrim(SX3->X3_CAMPO) $ "Z0_BATENTE.Z0_FECHELE.Z0_TESBAT.Z0_OBSBAT").Or.lBatEdit       //se segmento do cliente for diferente de Corporativo
